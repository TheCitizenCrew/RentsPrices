 @extends('layout') @section('title', 'Ajouter un loyer')
...
<div class="container-fluid">
	...
	@endif @if(empty($rent->id))
	<form class="form-horizontal" method="POST" action="/rent">
		@else
		<form class="form-horizontal" method="POST"
			action="/rent/{{$rent->id}}">
			@endif <input type="hidden" name="_token" id="csrf-token"
				value="{{ Session::token() }}" /> <input type="hidden" name="id"
				id="id" value="{{$rent->id}}" /> <input type="hidden"
				name="geolocManual" id="geolocManual"
				value="{{$rent->geolocManual}}" />

			<h2>Adresse</h2>

			<div
				class="form-group @if($errors->first('buildingIndividual'))has-error @endif">
				<label for="buildingIndividual" class="col-sm-1 control-label">Type</label>
				<div class="col-sm-6 checkbox">
					<label class="control-label"> <input type="radio"
						name="buildingIndividual" value="1" @if($rent->buildingIndividual==1)
						checked="checked" @endif />Logement collectif
					</label> <label class="control-label"> <input type="radio"
						name="buildingIndividual" value="0" @if($rent->buildingIndividual==='0')
						checked="checked" @endif />Logement individuelle
					</label> <span class="help-block">La bâtiment héberge-t-il un
						(individuel) ou plusieurs logements (collectif).</span>
				</div>
				@if ($errors->first('buildingIndividual'))
				<p class="text-danger">error
					{{$errors->first('buildingIndividual')}}</p>
				@endif
			</div>

			<div
				class="form-group @if($errors->first('buildingHLM'))has-error @endif">
			...
			<div
				class="form-group @if($errors->first('surfaceM2'))has-error @endif">
			...
			<div
				class="form-group @if($errors->first('roomsCount'))has-error @endif">
				<label for="roomsCount" class="col-sm-1 control-label">Nb.&nbsp;de
					pièces</label>
				<div class="col-sm-2">
					<input type="text" class="form-control" name="roomsCount"
						id="roomsCount" placeholder="Nombre de pièces du logement"
						value="{{$rent->roomsCount}}" />
				</div>
			...
			<div
				class="form-group @if($errors->first('kitchenRoom'))has-error @endif">
				<label for="kitchenRoom" class="col-sm-1 control-label">Une&nbsp;pièce
					cuisine</label>
				<div class="col-sm-5 checkbox">
					<label class="control-label"> <input type="radio"
			...
			<div
				class="form-group @if($errors->first('buildingStage'))has-error @endif">
			...
			<div
				class="form-group @if($errors->first('buildingName'))has-error @endif">
			...
			<p>
				La position géographique de l'adresse du logement a été calculée
				automatiquement. Vous pouvez la corriger en déplacement le marqueur
				bleu.<br /> <small>Le géocodage automatique ne fonctionne que pour
					la France métropolitaine, pour l'instant (cf. <a
					href="https://github.com/TheCitizenCrew/RentsPrices/issues/10">issue
						#10</a>).
				</small>
			</p>
			...
			<div id="rents">
				...
				<div id="rentRow{{$rowsCount}}" class="form-group">
					<input type="hidden" name="rentprice[{{$rowsCount}}][id]"
						value="{{$price->id}}" />

					<fieldset>
						<span class="col-sm-1 col-xs-1 control-label rentCount">
							{{$rowsCount + 1}} </span>
						<div class="col-sm-11">
							...
							<div class="form-group row">
								<div class="col-sm-2">
									<label>Année</label> <input type="text" class="form-control"
										placeholder="L'année" name="rentprice[{{$rowsCount}}][year]"
										value="{{$price->year}}" /> @if(
									$errors->first('year'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('year'.$rowsCount)}}</p>
									@endif
								</div>
								<div class="col-sm-2">
									<label>Mois</label> <input type="text" class="form-control"
										placeholder="Le mois" name="rentprice[{{$rowsCount}}][month]"
										value="{{$price->month}}" /> @if(
									$errors->first('month'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('month'.$rowsCount)}}</p>
									@endif
								</div>
								<div class="col-sm-2">
									<label>Loyer</label> <input type="text" class="form-control"
										placeholder="Le prix mensuel"
										name="rentprice[{{$rowsCount}}][price]"
										value="{{$price->price}}" /> @if(
									$errors->first('price'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('price'.$rowsCount)}}</p>
									@endif
								</div>
								<div class="col-sm-2">
									<label>Charges</label> <input type="text" class="form-control"
										placeholder="Les charges"
										name="rentprice[{{$rowsCount}}][loads]"
										value="{{$price->loads}}" /> @if(
									$errors->first('loads'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('loads'.$rowsCount)}}</p>
									@endif
								</div>
							</div>
							...
							<div class="form-group row">
								<div class="col-sm-2">
									...
									$errors->first('loadsOther'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('loadsOther'.$rowsCount)}}</p>
									@endif
								</div>
								<div class="col-sm-4">
									<label>Description autres Charges</label> <input type="text"
										class="form-control" placeholder="Description autres charges"
										name="rentprice[{{$rowsCount}}][loadsOtherText]"
										value="{{$price->loadsOtherText}}" />
									<p class="help-block">Par exemple "retenue pour investissement"</p>
									@if( $errors->first('loadsOtherText'.$rowsCount) )
									<p class="text-danger">error
										{{$errors->first('loadsOtherText'.$rowsCount)}}</p>
									@endif
								</div>
								<div class="col-sm-1">
									<button type="button" class="btn btn-default rentpriceTrash"
										data-rowcount="{{$rowsCount}}" aria-label="Left Align">
...
<button type="button" id="addRent" class="btn btn-default">
			...
			<br /> <a
				href="@if($rent->id > 0) {{ app('url')->route('RentShow',array('id'=>$rent->id)) }} @else {{ app('url')->route('Home') }} @endif"
